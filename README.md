...
Nunulo 邀请制服务已经上线，页面提供用户 Web、管理员后台、GitHub Organization 和 Android `v0.2.0` 固定 Release 资产入口。四角色权限、并发、治理、R2 对账与数据库恢复已经完成线上验证；物理 Android 仍需持续回归。
...
Android 包名仍为 `com.lumokato.nunulo`。当前页面固定指向稳定版 `v0.2.0`；只有对应 Release 资产实际存在后才算下载入口可用。稳定版不承担早期测试 APK 的原位升级兼容。
